...
def main():
    ...
    nf_model = util.init_flow_model(num_neurons, args.num_nf_layers, InterpRealNVP, ldr.train[0].shape[0], args)
    nf_optimizer = torch.optim.Adam([p for p in nf_model.parameters() if p.requires_grad], lr=args.lr)
    ...
    for epoch in range(args.n_epochs):
        util.endtoend_train(nf_model, nn_model, nf_optimizer, nn_optimizer, data_loader, args)  # Train the MCFlow model

        with torch.no_grad():
            ldr.mode = 1  # Use testing data
            te_mse, _ = util.endtoend_test(nf_model, nn_model, data_loader, args)  # Test MCFlow model
        ...
        if (epoch + 1) % reset_scheduler == 0:
            ...
            if args.dataset == 'mnist':
                ldr.reset_img_imputed_values(nn_model, nf_model, args)
            else:
                ldr.reset_imputed_values(nn_model, nf_model, args)
